...
contract Contractly {
    ...
    enum AgreementStatus { Pending, Active, Fulfilled, Breached, Disputed, Canceled }
    ...
    function createAgreement(
        ...
        bool _requiresAllPartiesToSign,
        bool _isStakingRequired,
    ...
    /**
     * @dev Mark an agreement as fulfilled (simplified for POC)
     * @param _agreementId The ID of the agreement
     */
    function fulfillAgreement(uint256 _agreementId) public agreementExists(_agreementId) {
        Agreement storage agreement = agreements[_agreementId];
        
        // In a real implementation, this would include verification logic or oracle integration
        // For POC purposes, we allow the contract owner to mark it fulfilled
        require(msg.sender == owner, "Only the system can mark as fulfilled");
    ...
    /**
     * @dev Mark an agreement as breached by a specific party
     * @param _agreementId The ID of the agreement
     * @param _breachingParty The address of the party that breached the agreement
     */
    function breachAgreement(uint256 _agreementId, address _breachingParty) public agreementExists(_agreementId) {
        Agreement storage agreement = agreements[_agreementId];
        
        // In a real implementation, this would include verification logic or oracle integration
        // For POC purposes, we allow the contract owner to mark it breached
        require(msg.sender == owner, "Only the system can mark as breached");
        require(agreement.status == AgreementStatus.Active, "Agreement must be active");
        
        agreement.status = AgreementStatus.Breached;
        
        // Handle breach consequences - for example, distribute the breaching party's funds
        ...
        if (breachingPartyStake > 0) {
            ...
            // In this simple POC, we'll distribute equally to non-breaching parties
            ...
            for (uint i = 0; i < nonBreachingParties.length; i++) {
    ...
    function isAgreementParty(uint256 _agreementId, address _address) public view agreementExists(_agreementId) returns (bool) {
        for (uint i = 0; i < agreements[_agreementId].parties.length; i++) {
            if (agreements[_agreementId].parties[i] == _address) {
                return true;
            }
        }
        return false;
